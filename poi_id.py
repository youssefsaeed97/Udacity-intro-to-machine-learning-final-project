...
features_list = ['poi', 'salary', 'total_payments', 'bonus', 'total_stock_value', 'expenses', 'exercised_stock_options', \
'from_poi_%', 'to_poi_%', 'shared_receipt_with_poi', \
'mean_top2000_80%tfidfsums']
...
estimators = [('pca', pca), ('scaler', scaler), ('Kbest', Kbest), ('DT',DT)]
pipe = Pipeline(estimators)
grid_param = {
                'Kbest__k': [10,],
                'pca__n_components': [10,],
                'DT__criterion': ('gini', 'entropy'),
                'DT__min_samples_split': [2, 8, 20]
        }
...
print(confusion_matrix(labels_test, pred, labels=[0, 1]), '\n')
print('precision:', precision_score(labels_test, pred, labels=[0, 1]), '\n')
print('recall:', recall_score(labels_test, pred, labels=[0, 1]), '\n')

print(classification_report(labels_test, pred, target_names=['non_poi', 'poi']))
